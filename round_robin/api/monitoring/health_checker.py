...
class HealthChecker:
    ...
    def health_check_loop(self):
        ...
        while self.running:
            ...
            print(
                "🔄 Checking health of instances and refreshing available ones...",
                flush=True,
            )
    ...
    def check_instances_health(self):
        ...
        for instance_url in list(self.failed_instances):  # Check only failed instances
            ...
            is_running = (
                self.docker_adapter.get_container_status(container_name) == "running"
            )
            ...
            if is_running and not is_overloaded and is_healthy:
                print(
                    f"{instance_url} has recovered! Removing from failure list.",
                    flush=True,
                )
                self.mark_failed(instance_url)
    ...
    def is_container_overloaded(self, container_name):
        ...
        print(
            f"🖥️ {container_name} → CPU: {cpu_usage}%, Memory: {memory_usage}%",
            flush=True,
        )

        return (
            cpu_usage > settings.CPU_THRESHOLD
            or memory_usage > settings.MEMORY_THRESHOLD
        )

    def mark_failed(self, instance_url):
        """Adds instance to the failed list (Used when request fails)"""
        print(
            f"❌ Marking {instance_url} as failed due to request failure!", flush=True
        )
        self.failed_instances.add(instance_url)
